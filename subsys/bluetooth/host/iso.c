...
static uint8_t get_free_big_handle(void)
{
	...
	return BT_ISO_INVALID_BIG_HANDLE;
}
...
static void cleanup_big(struct bt_iso_big *big)
{
	...
	big->handle = BT_ISO_INVALID_BIG_HANDLE;
}
...
{
	big->handle = get_free_big_handle();

	if (big->handle == BT_ISO_INVALID_BIG_HANDLE) {
		return -ENOMEM;
	}
	...
	for (int i = 0; i < big->num_bis; i++) {
		...
		if (!bis->qos ||
		    bis->qos->dir != (broadcaster ? BT_ISO_CHAN_QOS_OUT : BT_ISO_CHAN_QOS_IN)) {
...
static int hci_le_create_big(struct bt_le_ext_adv *padv, struct bt_iso_big_create_param_t *param)
{
...
int bt_iso_big_create(struct bt_le_ext_adv *padv, struct bt_iso_big_create_param_t *param)
{
	...
	if (big->handle) {
		BT_DBG("BIG handle already initialized %u", big->handle);
		return -EALREADY;
	}
	...
	err = hci_le_create_big(padv, param);
	if (err) {
...
static int hci_le_terminate_big(struct bt_iso_big *big)
{
	...
	struct bt_hci_evt_le_big_terminate *evt;
	struct net_buf *buf;
	struct net_buf *rsp;
	int err;
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_LE_TERMINATE_BIG, buf, &rsp);
	if (err) {
		return err;
	}

	evt = (struct bt_hci_evt_le_big_terminate *)rsp->data;
	if (evt->big_handle != big->handle) {
		err = -EIO;
	}
...
int bt_iso_big_terminate(struct bt_iso_big *big)
{
	...
	if (big->handle == BT_ISO_INVALID_BIG_HANDLE || !big->num_bis || !big->bis) {
	...
	broadcaster = big->bis[0]->qos->dir == BT_ISO_CHAN_QOS_OUT;
...
{
	struct bt_hci_evt_le_big_complete *evt = (struct bt_hci_evt_le_big_complete *)buf->data;
	struct bt_iso_big *big = bigs[evt->big_handle];

	BT_DBG("BIG[%u] completed, status %u", big->handle, evt->status);
...
void hci_le_big_termimate(struct net_buf *buf)
{
	...
	BT_DBG("BIG[%u] terminated", big->handle);
...
void hci_le_big_sync_established(struct net_buf *buf)
{
	...
	if (evt->status || evt->num_bis != big->num_bis) {
		...
		for (int i = 0; i < big->num_bis; i++) {
			...
			if (bis->ops->disconnected) {
				bis->ops->disconnected(bis);
			}
		}
		return;
	}
...
void hci_le_big_sync_lost(struct net_buf *buf)
{
	...
	BT_DBG("BIG[%u] sync lost", big->handle);
...
static int hci_le_big_create_sync(const struct bt_le_per_adv_sync *sync, struct bt_iso_big *big,
				  const struct bt_iso_big_sync_param_t *param)
{
		    ...
		    struct bt_iso_big_sync_param_t *param)
{
	...
	if (big->handle) {
		BT_DBG("BIG handle already allocated %u", big->handle);
		return -EALREADY;
	}
